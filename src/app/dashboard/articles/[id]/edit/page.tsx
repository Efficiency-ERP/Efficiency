      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {form.type === "product" && <div className="grid gap-2"><Label>Unité</Label><Input value={form.unit} onChange={(e) => setForm({ ...form, unit: e.target.value })} /></div>}
          </CardContent>
        </Card>
        <Card>
            ...
            <CardContent className="space-y-4">
              {packaging.map((pkg, i) => (
                <div key={i} className="flex gap-4 items-end">
                  ...
                  <Button type="button" variant="destructive" size="sm" onClick={() => setPackaging(packaging.filter((_, idx) => idx !== i))}>Supprimer</Button>
                </div>
              ))}
              <Button type="button" variant="outline" onClick={() => setPackaging([...packaging, { type: "BOUTEILLE", unitsPerArticle: 12, depositValue: 1.5 }])}>Ajouter un emballage</Button>
            </CardContent>
          </Card>
        )}
