...
import { FileText, CreditCard, ArrowDownRight, Truck, Package } from "lucide-react"

export default function CreateInvoiceChooserPage() {
  const router = useRouter()

  const options = [
    { label: "Quote (Devis)", type: "quote", icon: FileText, variant: "default" as const },
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-muted-foreground">
        Sales invoices come from a Quote (Q → I). Credit/Debit notes adjust an existing invoice.
        Warehouse stock corrections live under Stock, not here — they never touch money.
      </p>
            ...
            <Card key={opt.type} className="cursor-pointer hover:border-primary transition-colors">
              <CardContent className="flex flex-col items-center gap-3 p-6">
                ...
                <Button variant={opt.variant} onClick={() => {
                  ...
                  else if (opt.type === "quote") router.push("/dashboard/quotes/create")
